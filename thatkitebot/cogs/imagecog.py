...
from thatkitebot.base.image_stuff import get_image_url, ImageFunction, ImageTooLarge
...
class ImageStuff(commands.Cog, name="image commands"):
        ...
        async with self.sep:
            try:
                ...
                # for some reason it never runs the function if you use an actual executor, so this is a temporary workaround
                    await asyncio.wait_for(self.ll.run_in_executor(executor=None, func=func), timeout=30.0)
                else:
                    b2, fn = await asyncio.wait_for(self.ll.run_in_executor(None, func), timeout=30.0)
    ...
    image_group = discord.SlashCommandGroup("image", "Image Commands", guild_ids=[759419755253465188])

    @image_group.command(guild_ids=[759419755253465188])
    @option("Image", discord.Attachment, description="The image you want to process", required=False)
    async def magik(self, ctx: discord.ApplicationContext, attachment: Optional[discord.Attachment]):
        """
        Applies some content aware scaling to an image.
        ...
        image = ImageFunction(buf, 0)
        await self.image_worker(image.magik)
        embed, file = await self.image_worker(image.make_blob_close, does_return=True, name="magik")
    ...
    #     async with ctx.channel.typing():
    #         embed, file = await self.image_worker(
    #             functools.partial(magik.bubble, buf=buf, fn=17, flip=flip), "speech_bubble", gif=True)
    #         buf.close()
    #     await ctx.send(file=file, embed=embed)
    #
    # @commands.cooldown(5, 10, commands.BucketType.user)
