...
class FieldModal(discord.ui.Modal):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        self.config[self.section_id]["fields"][self.field_id]["value"] = self.children[1].value # set value of the field
        ...
        await interaction.response.send_message(f"Field with ID {self.field_id + 1} in section {self.config[self.section_id]['title']} has been changed.")
...
class InfoCog(commands.Cog, name="Info"):
    ...
    async def get_sections(self, ctx: discord.AutocompleteContext):
        return [f"{id+1}. {section['title']}" for id, section in enumerate(await self.get_config(ctx.interaction.guild)) 
    ...
    async def get_dropdown(self, guild):
        ...
        for id, section in enumerate(await self.get_config(guild)):
            if discord_emoji.to_uni(section["emoji"]): # if it's standard emoji (e.g. :smile:) it has to be converted to unicode
                option_list.append(discord.SelectOption(label=f'{id+1}. {section["title"]}', emoji= discord_emoji.to_uni(section["emoji"])))
            else:
    ...
    async def edit_field(self, ctx: discord.ApplicationContext, section: Option(str, "Pick a section!", required = True, autocomplete=get_sections), 
        ...
        if option == "add":
            ...
            modal = FieldModal(title=f"Editing new field :pencil:", 
                                config=await self.get_config(ctx.guild), section_id=id, field_id=len(config[id]["fields"])-1, bot=self.bot)
    ...
    async def edit_section(self, ctx: discord.ApplicationContext, section: Option(str, "Choose a section", required = True, autocomplete=get_sections), 
    ...
    async def info_dropdown_callback(self, interaction: discord.Interaction):
        ...
        await interaction.response.edit_message(embed=await get_embed(id, await self.get_config(interaction.guild)), view=self.main_view, content=None)
    ...
    async def edit_field_dropdown_callback(self, interaction: discord.Interaction):
        ...
        modal = FieldModal(title=f"Field editor :pencil:", 
                            config=await self.get_config(interaction.guild), section_id=id, field_id=field_id, bot=self.bot)
    ...
    async def remove_field_dropdown_callback(self, interaction: discord.Interaction):
        ...
        await interaction.response.edit_message(content=f"Field no. {field_id} has been removed from section {name} {emoji}.", view= None)
