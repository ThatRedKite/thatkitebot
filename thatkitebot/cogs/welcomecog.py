...
class WelcomeCog(commands.Cog, name="Welcome counter"):
    ...
    async def cog_check(self, ctx):
        return await RedisFlags.get_guild_flag(self.settings_redis, ctx.guild, RedisFlags.WELCOME)
    ...
    async def on_message(self, message: discord.Message):
        ...
        if not await RedisFlags.get_guild_flag(self.settings_redis, message.guild, RedisFlags.WELCOME):
            return
    ...
    async def on_member_join(self, joined_member):
        ...
        self.bot.events_total += 1
        # check, if welcome features are even enabled
        if not await RedisFlags.get_guild_flag(self.settings_redis, joined_member.guild, RedisFlags.WELCOME):
            return
        ...
        if await RedisFlags.get_guild_flag(self.settings_redis, joined_member.guild, RedisFlags.WELCOME_MESSAGE):
