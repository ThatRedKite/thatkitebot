...
class issues(commands.Cog):
    ...
    def new_issue(self, session: Session, author_id: int, author_name: str, message: str, status: str = "open",
                  hidden: bool = False):
        nowtime = int(time())
        info = dict(
            timestamp=nowtime,
    ...
    async def get(self, ctx, id: str):
        ...
        embed.add_field(name="author", value=f"`{issue_dict.get('author_id')}({issue_dict.get('author_name')})`",
                        inline=True)
        ...
        if testhash == id:
            ...
            embed.set_footer(
                text=f"WARNING INTEGRITY CHECK FAILED! HASH VALUES DON'T MATCH!!!\n**{testhash}** vs **{id}**")
